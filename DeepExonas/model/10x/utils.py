...
def z_leiden_cluster(out_path, model, adata, data_loader, epoch, device):
    ...
    for res in np.arange(0.0,2,0.1):
        ...
        ARI = adjusted_rand_score(adata.obs["celltype1"].tolist(), adata.obs["leiden"].tolist())
    ...
    # scib.me.cluster_optimal_resolution(adata, cluster_key="leiden", label_key="celltype1") #score here is corresponding to NMI
    # celltype_l1_ari = scib.me.ari(adata, cluster_key="leiden", label_key="celltype1")
    with plt.rc_context():
        sc.pl.umap(adata, color=['leiden', "celltype1", "celltype2"], wspace=0.5, title='celltype1_ARI='+ str(round(max(celltype1_ari_val_list), 3)) + ", Epoch:" + str(epoch) + ", Sample_type: " + sample_type, show=False)
    ...
    for res in np.arange(0.0,2,0.1):
        ...
        sc.tl.leiden(adata, resolution=round(res,1))
        ARI = adjusted_rand_score(adata.obs["celltype2"].tolist(), adata.obs["leiden"].tolist())
        #store ari for speicific sample type
    ...
    #celltype
    # scib.me.cluster_optimal_resolution(adata, cluster_key="leiden", label_key="celltype2") #score here is corresponding to NMI
    # celltype_l1_ari = scib.me.ari(adata, cluster_key="leiden", label_key="celltype2")
    with plt.rc_context():
        sc.pl.umap(adata, color=['leiden', "celltype1", "celltype2"], wspace=0.5, title='celltype2_ARI='+ str(round(max(celltype2_ari_val_list), 3)) + ", Epoch:" + str(epoch) + ", Sample_type: " + sample_type, show=False)
