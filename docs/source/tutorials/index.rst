Tutorials
=================

This section contains step-by-step tutorials for using the DOLPHIN tool.

.. toctree::
    :maxdepth: 1
    
    step0_generate_exon_gtf.ipynb
    step1_1_preprocess_full_length.md
    step1_2_preprocess_10X.md
    step2_1_graph_generation.ipynb
    step2_2_exonGene_matrix_generation.ipynb
    step2_3_feature_matrix_generation.ipynb
    step2_4_adjacency_matrix_generation.ipynb
    step2_5_graph_data_generation.ipynb
    step3_run_DOLPHIN.ipynb
    step4_cell_aggregation.ipynb
    step5_alternative_splicing.md
    step6_alternative_splicing_analysis.ipynb
    step7_1_MAST.ipynb
    step7_2_EDEG_analysis.ipynb