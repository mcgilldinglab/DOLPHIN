...
def combine_fea(pbar, pd_gt, graph_path, gene_annotation, gtf_pkl_path, start_idx, sample_num, output_path, output_name):
    ...
    # '''
    # Step4: get ground truth cell type, sample names
    # '''
    # #rename all the cell barcodes
    # # df_label = pd.read_csv(sample, sep='\t', names=["CB", "celltype"])

    '''
    Step5: combine all feature matrix together per sample id
    '''
    cell_fea = np.array([[]])
    _df_temp = pd_gt
