...
def combine_fea(pbar, pd_gt, graph_path, gene_annotation, gtf_pkl_path, start_idx, sample_num, output_path, output_name):
    """
    Combines exon-level read counts generated by FeatureCounts and converts them into a feature matrix (AnnData) representing an exon graph.

    This function reads gene annotation and GTF data to construct annotated gene-exon names,
    loads per-cell exon feature matrices from disk, merges them with corresponding cell metadata,
    and builds an AnnData object which is saved as an `.h5ad` file for downstream analysis.

    Parameters
    ----------
    pbar : tqdm.tqdm
        A tqdm progress bar object to visually track the progress.
    pd_gt : pandas.DataFrame
        A DataFrame containing ground truth metadata per cell, including cell barcodes and labels.
    graph_path : str
        Path to the directory containing exon read files generated from featurecounts
    gene_annotation : str
        Path to the gene annotation CSV file, which maps gene IDs to gene names.
    gtf_pkl_path : str
        Path to a pickled GTF DataFrame containing gene information.
    start_idx : int
        Starting index (zero-based) in the cell list to begin processing.
    sample_num : int
        Number of cells (samples) to process from the starting index.
    output_path : str
        Directory to which the resulting `.h5ad` file will be saved.
    output_name : str
        Prefix name to use for the output `.h5ad` file.

    Returns
    -------
    pbar : tqdm.tqdm
        The updated progress bar after processing the given cells.

    Notes
    -----
    - The GTF DataFrame is expected to have columns: "seqname", "gene_id", "start", "end".
    - Per-cell feature files must exist and be named consistently using the cell barcode.
    - The output file is named using the format: `Feature_<output_name>_<batch_index>.h5ad`.
