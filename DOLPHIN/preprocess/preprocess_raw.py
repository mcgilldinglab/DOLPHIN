...
"""
class gene(object):
    def __init__(self, main_folder, gtf, adj_ind, srr, id = "", show = "N", ck = "N"):
    ...
    def get_gene(self):
        ...
        df_temp_exon = pd.read_csv(os.path.join(self.main_folder, "05_exon_junct_cnt", self.srr + ".exon.count.txt"), skiprows = 1, sep = '\t',low_memory=False)
    ...
    """
    def jun_read(self):
        df_temp_sj = pd.read_csv(os.path.join(self.main_folder, "05_exon_junct_cnt", self.srr + ".exon.count.txt.jcounts"), skiprows = 0, sep = '\t')
    ...
    def count_adj(self):
        ...
        _out_list = [[nan,nan] for _ in range(len(_jun_list))] #output adjacent list
        ...
        #delete any edges if there is only one node
            ...
            """
            This is the bug need to be fixed at the next version, 2023-02-22
            but if the feature matrix has count = 0 at this node, which cannot assign to 1, should be zero, 
        ...
        #standardization
        # if _df_adj_sum.empty:
        #     self.adj_mat = am.flatten()
        # else:
        #     self.adj_mat = np.round(am/np.sum(am),4).flatten()
        self.adj_mat = am.flatten()
    ...
    def get_all(self):
        if self.id == "":
            ...
            np.savetxt(os.path.join(self.main_folder, "06_graph_mtx", self.srr+ "_fea.csv"), vec_f, fmt='%10.4f', delimiter = ',')
            np.savetxt(os.path.join(self.main_folder, "06_graph_mtx", self.srr+ "_adj.csv"), vec_a, fmt='%10.4f', delimiter = ',')
