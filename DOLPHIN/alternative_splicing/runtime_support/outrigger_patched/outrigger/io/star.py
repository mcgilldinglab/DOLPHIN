...
def read_sj_out_tab(filename):
    ...
    """
    sj = pd.read_table(filename, header=None, names=COLUMN_NAMES, sep=r'\s+')
    sj[JUNCTION_MOTIF] = sj[JUNCTION_MOTIF].map(int_to_junction_motif)

    # Convert unknown strands to explicitly say "undefined".
    # On newer pandas versions, assigning '+'/'-' into an int64 column raises
    # instead of silently widening the dtype, so force and keep an object dtype.
    sj[STRAND] = sj[STRAND].astype(object)
    rows = sj[STRAND] == 0
    sj.loc[rows, STRAND] = 'undefined'
