...
class Index(Subcommand):
    ...
    def build_db_with_novel_exons(self, novel_exons_gtf, db):
        if not os.path.exists(novel_exons_gtf) or os.path.getsize(novel_exons_gtf) == 0:
            return db

        merged_gtf = os.path.join(self.gtf_folder,
                                  os.path.basename(self.gtf_filename))
        util.progress('Writing merged GTF with novel exons to {} ...'.format(
            merged_gtf))
        with open(merged_gtf, 'w') as out:
            with open(self.gtf_filename) as original:
                shutil.copyfileobj(original, out)
            with open(novel_exons_gtf) as novel:
                first = novel.read(1)
                if first:
                    out.write('\n')
                    out.write(first)
                    shutil.copyfileobj(novel, out)
        util.done()

        merged_db = os.path.join(
            self.gtf_folder, '{}.db'.format(os.path.basename(merged_gtf)))
        if os.path.exists(merged_db):
            os.remove(merged_db)
        util.progress('Building gffutils database with novel exons at {} ...'
                      .format(merged_db))
        db = gtf.create_db(merged_gtf, merged_db)
        pragmas = dict(gffutils.constants.default_pragmas)
        pragmas['journal_mode'] = 'WAL'
        db.set_pragmas(pragmas)
        util.done()
    ...
    def make_exon_junction_adjacencies(self, metadata, db):
        ...
        novel_exons_gtf = os.path.join(self.gtf_folder, 'novel_exons.gtf')
        n_novel_exons = len(exon_junction_adjacencies.novel_exon_features)
        ...
        db = self.build_db_with_novel_exons(novel_exons_gtf, db)
        exon_junction_adjacencies = adjacencies.ExonJunctionAdjacencies(
            metadata, db, max_de_novo_exon_length=self.max_de_novo_exon_length)
        ...
        return junction_exon_triples, db
    ...
    def make_events_by_traversing_graph(self, event_maker, db):
        ...
        for splice_abbrev, event_df in event_dfs.items():
            ...
            n_events = len(event_df.groupby(level=0))
            if n_events > 0:
                util.progress(
    ...
    def execute(self):
        ...
        junction_exon_triples, db = self.make_exon_junction_adjacencies(
            metadata, db)
...
class Validate(SubcommandAfterIndex):
    ...
    def execute(self):
        ...
        for splice_name, splice_abbrev in common.SPLICE_TYPES:
            ...
            n_total = len(splice_sites.groupby(level=0))
            ...
            n_valid = len(splice_sites_validated.groupby(level=0))
