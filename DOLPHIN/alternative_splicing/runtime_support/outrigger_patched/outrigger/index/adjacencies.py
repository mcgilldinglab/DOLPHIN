...
class ExonJunctionAdjacencies(object):
    ...
    def detect_exons_from_junctions(self):
        ...
        for chrom, df in junctions.groupby('chrom'):
            ...
            progress('\t\tCollecting {n} novel exons on '
            ...
            if not exon_features:
                ...
                self.novel_exon_features.extend(exon_features)
                self.existing_exons.update(feature.id for feature in exon_features)
            done(n_tabs=4)
    ...
    def write_de_novo_exons(self, filename='novel_exons.gtf'):
        ...
        with open(filename, 'w') as f:
            novel_exons = sorted(
                self.novel_exon_features,
                key=lambda exon: (exon.seqid, exon.start, exon.end,
                                  exon.strand, exon.id),
            )
