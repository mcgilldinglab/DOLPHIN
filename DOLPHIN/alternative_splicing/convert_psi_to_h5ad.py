...
def run_convert_psi(
    ...
    # Preserve the original event order from the Outrigger summary while avoiding
    # repeated full-table filtering per cell, which is prohibitively slow on 10x runs.
    event_order = pd.Index(pd_psi_single["event_id"].drop_duplicates())
    df_recon = pd_psi_single.pivot_table(
        index="sample_id",
        columns="event_id",
        values="psi",
        aggfunc="first",
    ...
    df_recon = df_recon.reindex(index=all_sample, columns=event_order)
    df_obs_org = df_meta.set_index("CB").join(df_recon, how="left")
    ...
    write_h5ad_preserve_strings(adata, os.path.join(final_out_dir, out_name+"_PSI.h5ad")) 
